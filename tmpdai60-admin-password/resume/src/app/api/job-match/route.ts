...
export async function GET(request: Request) {
  ...
  const res = await callBackend(`/api/job-match${qs ? `?${qs}` : ""}`, { authToken: token });
...
export async function POST(request: Request) {
  ...
  const res = await callBackend("/api/job-match", { method: "POST", authToken: token, forwardRequest: request });
