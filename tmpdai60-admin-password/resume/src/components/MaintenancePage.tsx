...
// Full-screen takeover shown when a backend proxy call returns 5xx (see MaintenanceProvider).
// The copy depends on *why* the backend is unreachable:
//   • inside the scheduled 2:00–5:00 AM (GMT+7) window → planned maintenance message
//   • outside the window → a generic "unexpected server error" message, since claiming
//     scheduled maintenance would be misleading.
export default function MaintenancePage({ onRetry }: { onRetry: () => void }) {
  const { t } = useTranslation();
  const scheduled = isMaintenanceWindow();

  const title = scheduled ? t("maintenance.pageTitle") : t("maintenance.errorTitle");
  const body = scheduled ? t("maintenance.pageBody") : t("maintenance.errorBody");
